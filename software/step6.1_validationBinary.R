...
if(is.null(opt$results_file)    )   {opt$results_file = "./topBinary_Sep2018.txt"}
...
foreach(i = 1:nrow(data),.combine = rbind) %do% {
  ...
  if ((data$ST.bac.name[i] %in% colnames(microbe)) & (data$SNPName[i] %in% colnames(dosages)) ) {
    bac = microbe[,data$ST.bac.name[i]]
    dos = dosages[,data$SNPName[i]]
    glm1 = glm(bac ~ dos + ., data = phenotypes,family = "binomial")
    summary1 = summary(glm1)
    result = data.frame(bac = data$ST.bac.name[i],
                        snp = data$SNPName[i],
                        ref.allele = genotypes[which.min(dosages[,data$SNPName[i]]),data$SNPName[i]],
                        alt.allele = genotypes[which.max(dosages[,data$SNPName[i]]),data$SNPName[i]],
